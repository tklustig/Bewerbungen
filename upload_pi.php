...
<form action=<?= $_SERVER['PHP_SELF']; ?> name="formular" id="formular" method="post" onsubmit="return checkForm()" enctype="multipart/form-data">
    ...
    <input class="button1" type=text name="betreff" id="betreff" value="<?php if (!empty($_REQUEST["betreff"])) echo $_REQUEST["betreff"]; ?>"><br>
     <!--<p>Referenznummer</p>
    <input class="button1" type=text name="referenz" id="ref" value="<?php // if (!empty($_REQUEST["referenz"])) echo $_REQUEST["referenz"];  ?>"><br><br><br> -->
    <label>Die Mailadresse der Firma:</label>
    <?php
    echo'<label><font color="#FF00FF">' . $_SESSION["mail"] . '</label></font><br>';
    ?>
    <label>Ihre Mailadresse:</label>
    <?php
    ...
    <label for="anrede_0"><?php echo $_SESSION["anrede"] ?> <?php echo $_SESSION["bl"] ?>,<br><div style="text-indent:25px;">hiermit bewerbe ich mich auf Ihre ausgeschriebene Stelle als Softwareentwickler.  <?php //echo $_REQUEST["referenz"]  ?>
            <div style="text-indent:25px;">Das Anschreiben sowie meine Referenzen sind komplett als Anhang beigefuegt.<br><br><div style="text-indent:25px;">Mit freundlichen Gruessen<br><div style="text-indent:25px;">Thomas Kipp<br><br></label></div></div></div>
            ...
            //$_SESSION["referenz"] = $_REQUEST["referenz"];
            ...
            if (!empty($_REQUEST["mail_send"]) && $_REQUEST["mail_send"] == "Bewerben") {
                ...
                echo "Betreff:" . $_REQUEST["betreff"] . "<br><br>";
                echo nl2br($show_mail);
